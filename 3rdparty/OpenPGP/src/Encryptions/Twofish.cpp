...
std::string Twofish::run(const std::string & data, bool enc) {
    ...
    if (enc) {
        for(uint8_t i = 0; i < 8; i++) {
            t1 = mk_tab[0][extract_byte(blk[1],3)] ^ mk_tab[1][extract_byte(blk[1],0)] ^ mk_tab[2][extract_byte(blk[1],1)] ^ mk_tab[3][extract_byte(blk[1],2)];
            t0 = mk_tab[0][extract_byte(blk[0],0)] ^ mk_tab[1][extract_byte(blk[0],1)] ^ mk_tab[2][extract_byte(blk[0],2)] ^ mk_tab[3][extract_byte(blk[0],3)];
            blk[2] = ROR(blk[2] ^ (t0 + t1 + l_key[4 * i + 8]), 1, 32);
            blk[3] = ROL(blk[3], 1, 32) ^ (t0 + 2 * t1 + l_key[4 * i + 9]);
            t1 = mk_tab[0][extract_byte(blk[3],3)] ^ mk_tab[1][extract_byte(blk[3],0)] ^ mk_tab[2][extract_byte(blk[3],1)] ^ mk_tab[3][extract_byte(blk[3],2)];
            t0 = mk_tab[0][extract_byte(blk[2],0)] ^ mk_tab[1][extract_byte(blk[2],1)] ^ mk_tab[2][extract_byte(blk[2],2)] ^ mk_tab[3][extract_byte(blk[2],3)];
...
void Twofish::setkey(const std::string & KEY) {
    ...
    if (k_len == 2) {
        for (uint16_t i = 0; i < 256; i++) {
            mk_tab[0][i] = m_tab[0][q_tab[0][q_tab[0][i] ^ extract_byte(s_key[1],0)] ^ extract_byte(s_key[0],0)];
            mk_tab[1][i] = m_tab[1][q_tab[0][q_tab[1][i] ^ extract_byte(s_key[1],1)] ^ extract_byte(s_key[0],1)];
            mk_tab[2][i] = m_tab[2][q_tab[1][q_tab[0][i] ^ extract_byte(s_key[1],2)] ^ extract_byte(s_key[0],2)];
            mk_tab[3][i] = m_tab[3][q_tab[1][q_tab[1][i] ^ extract_byte(s_key[1],3)] ^ extract_byte(s_key[0],3)];
        }
    } else if (k_len == 3) {
        for (uint16_t i = 0; i < 256; i++) {
            mk_tab[0][i] = m_tab[0][q_tab[0][q_tab[0][q_tab[1][i] ^ extract_byte(s_key[2], 0)] ^ extract_byte(s_key[1], 0)] ^ extract_byte(s_key[0], 0)];
            mk_tab[1][i] = m_tab[1][q_tab[0][q_tab[1][q_tab[1][i] ^ extract_byte(s_key[2], 1)] ^ extract_byte(s_key[1], 1)] ^ extract_byte(s_key[0], 1)];
            mk_tab[2][i] = m_tab[2][q_tab[1][q_tab[0][q_tab[0][i] ^ extract_byte(s_key[2], 2)] ^ extract_byte(s_key[1], 2)] ^ extract_byte(s_key[0], 2)];
            mk_tab[3][i] = m_tab[3][q_tab[1][q_tab[1][q_tab[0][i] ^ extract_byte(s_key[2], 3)] ^ extract_byte(s_key[1], 3)] ^ extract_byte(s_key[0], 3)];
        }
    } else if (k_len == 4) {
        for (uint16_t i = 0; i < 256; i++) {
            mk_tab[0][i] = m_tab[0][q_tab[0][q_tab[0][q_tab[1][q_tab[1][i] ^ extract_byte(s_key[3], 0)] ^ extract_byte(s_key[2], 0)] ^ extract_byte(s_key[1], 0)] ^ extract_byte(s_key[0], 0)];
            mk_tab[1][i] = m_tab[1][q_tab[0][q_tab[1][q_tab[1][q_tab[0][i] ^ extract_byte(s_key[3], 1)] ^ extract_byte(s_key[2], 1)] ^ extract_byte(s_key[1], 1)] ^ extract_byte(s_key[0], 1)];
            mk_tab[2][i] = m_tab[2][q_tab[1][q_tab[0][q_tab[0][q_tab[0][i] ^ extract_byte(s_key[3], 2)] ^ extract_byte(s_key[2], 2)] ^ extract_byte(s_key[1], 2)] ^ extract_byte(s_key[0], 2)];
            mk_tab[3][i] = m_tab[3][q_tab[1][q_tab[1][q_tab[0][q_tab[1][i] ^ extract_byte(s_key[3], 3)] ^ extract_byte(s_key[2], 3)] ^ extract_byte(s_key[1], 3)] ^ extract_byte(s_key[0], 3)];
        }
    }
